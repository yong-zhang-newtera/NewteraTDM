...
{
    public class CNTKModelEvaluator
    {
        /// <summary>
        /// Get result of evaluating a model
        /// </summary>
        /// <param name="modelFilePath">the model file path</param>
        /// <param name="features">Semi-colon separated features</param>
        /// <param name="labels">Semi-colon separated labels</param>
        /// <param name="device">Specify on which device to run the evaluation.</param>
        public static async Task<Dictionary<Variable, Value>> EvaluateModel(string modelFilePath, string features, string labels, DeviceDescriptor device)
        {
            Dictionary<Variable, Value> result = new Dictionary<Variable, Value>();

            await Task.Factory.StartNew(() =>
            {
                result = GetEvaluateResult(modelFilePath, features, labels, device);
            });
        ...
        /// <param name="device">Specify on which device to run the evaluation.</param>
        public static async Task<MLModelInfo> GetModelInfo(string modelFilePath, string modelName, DeviceDescriptor device)
        {
            MLModelInfo result = null;

            await Task.Factory.StartNew(() =>
            {
                result = RetrieveModelInfo(modelFilePath, modelName, device);
            });
        ...
        public static IList<IList<float>> EvaluateTimeSeriesModel(string modelFilePath, List<string> featureList, DeviceDescriptor device)
        {
            try
            {
                ...
                var sequence = GetSequenceFromOutputvar(outputDataMap, labelDimension);
                ...
                IList<IList<float>> sequenceList = GetSequenceList(sequence, labelDimension);
        ...
        private static IList<float> GetSequenceFromOutputvar(Dictionary<Variable, Value> outputMap, int labelDimension)
        {
            IList<float> sequence = null;
        ...
        internal static Dictionary<Variable, Value> GetEvaluateResult(string modelFilePath, string features, string labels, DeviceDescriptor device)
        {
            try
            {
                // Load the model.
        ...
        internal static MLModelInfo RetrieveModelInfo(string modelFilePath, string modelName, DeviceDescriptor device)
        {
            try
            {
                ...
                // get input info
                IList<Variable> inputVars = modelFunc.Arguments;
                MLVariableInfo mlVariable;
                foreach (Variable inputVar in inputVars)
                {
                    mlVariable = new MLVariableInfo();
                    mlVariable.Name = inputVar.Name;
                    mlVariable.DataType = inputVar.DataType.ToString();
                    mlVariable.Dimension = inputVar.Shape.TotalSize;

                    modelInfo.InputVariables.Add(mlVariable);
                }
                ...
                {
                    mlVariable = new MLVariableInfo();
                    mlVariable.Name = outputVar.Name;
                    mlVariable.DataType = outputVar.DataType.ToString();
                    mlVariable.Dimension = outputVar.Shape.TotalSize;

                    modelInfo.OutputVariables.Add(mlVariable);
                }
