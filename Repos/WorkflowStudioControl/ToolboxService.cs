...
namespace Newtera.WorkflowStudioControl
{
	...
	public class ToolboxService: Panel, IToolboxService
	{
        ...
        private string _selectedCategory = CATEGORY_ALL; // default to show all activities
		...
		public string SelectedCategory
		{
			...
			set
			{
                if (value != _selectedCategory)
                {
                    ...
                    // refresh the tool box to show the activities of given category
		...
		private void AddToolboxEntries(ListBox lb)
		{
			...
			while (end < entries.Length)
			{
				...
				if (entry.Length != 0 && !entry.StartsWith(";"))
                {
                    ...
                    string category = entry.Substring(index+1);
                    if (this._selectedCategory == CATEGORY_ALL ||
