...
namespace Newtera.WebApi.Infrastructure
{
        ...
        where TUser : ApplicationUser
    {
        ...
        public UserStore()
        {
            new UserStore<TUser>(new ServerSideUserManager());
        }
        ...
        public UserStore(IUserManager customUserManager)
        {
            ...
            userTable = new UserTable<TUser>(customUserManager);
            roleTable = new RoleTable(customUserManager);
            userRolesTable = new UserRolesTable(customUserManager);
            userClaimsTable = new UserClaimsTable(customUserManager);
            userLoginsTable = new UserLoginsTable(customUserManager);
        }
        ...
        /// <returns></returns>
        public Task<TUser> FindAsync(string userName, string password)
        {
            var theUserName = userTable.FindUserNameByCredential(userName, password);
            if (theUserName != null)
            {
                TUser user = userTable.GetUserByName(theUserName) as TUser;
                if (user != null)
                {
        ...
        {
            userRolesTable.DeleteRole(user.Id, role);
            return Task.FromResult<Object>(null);
        }
        ...
        {
            userTable.SetPasswordHash(user.UserName, passwordHash);
