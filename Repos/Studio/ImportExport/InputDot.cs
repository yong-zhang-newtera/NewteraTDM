...
namespace Newtera.Studio.ImportExport
{
	...
	/// <summary>
	/// A UI component representing an input dot of a rectangle component. It can be connected to
	/// a source grid item using a line.
	/// </summary>
	/// <version> 1.0.0 10 Nov 2004 </version>
	/// <author> Yong Zhang</author>
	public class InputDot : MapComponentBase
	{
		...
		/// <summary>
		/// Initiate an instance of InputDot
		/// </summary>
		/// <param name="panel">The panel on which to draw lines</param>
		public InputDot(MapPanel panel, MapRect rect) : base(panel)
		{
			_rect = rect;
		}
		...
		public int YCoordinate
		{
			get
			{
				return _rect.Y + (MapRect.RectHeight - InputDot.HEIGHT) / 2;
			}
		}
		...
		internal MapComponentCollection InputLines
		{
			get
			{
				return _rect.InputLines;
			}
		}

		/// <summary>
		/// Gets mapping type to which the input dot is associated with
		/// </summary>
		...
		public override MapComponentType ComponentType
		{
			get
			{
				return MapComponentType.InputEnd;
			}
		}
		...
		public override bool Hits(int x, int y)
		{
			...
			if (x >= XCoordinate && x < XCoordinate + InputDot.WIDTH &&
				y >= YCoordinate && y < (YCoordinate + InputDot.HEIGHT))
			{
				status = true;
			}
				...
				using (Graphics g = this._panel.CreateGraphics())
				{
					if (!IsSelected)
					{
						...
						g.FillRectangle(Brushes.DarkGray,
							...
							InputDot.WIDTH, /* width */
							InputDot.HEIGHT /* height */);
					}
					else
					{
