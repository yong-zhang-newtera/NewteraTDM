...
namespace Newtera.Studio.ImportExport
{
	...
	public class MapManyToMany : MapRect
	{
		/// <summary>
		/// Initiate an instance of MapManyToMany
		/// </summary>
		/// <param name="panel">The panel on which to draw lines</param>
		public MapManyToMany(MapPanel panel) : base(panel)
		{
		}
		...
		public override MapComponentType ComponentType
		{
			get
			{
				return MapComponentType.ManyToMany;
			}
		}
		...
		{
			_mapping = new ManyToManyMapping();
			_mapping.X = this.X;
			_mapping.Y = this.Y;
