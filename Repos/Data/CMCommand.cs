...
namespace Newtera.Data
{
	...
	public sealed class CMCommand : IDbCommand
	{
        ...
        /// <summary>
        /// Get time series in xml for the data instance using the provided xml schema
        /// </summary>
        ...
        /// <param name="category">The category of a time series, can be null</param>
        /// <param name="fieldName">Specify the field name to get, get all seieses if null</param>
        /// <param name="frequency">Resample frequency, such as Second, Minute, Hour, ect.</param>
        /// <param name="op">Resample operation, such as min,max, mean, etc</param>
        /// <param name="startRow">The start row index, -1 for all records</param>
        /// <param name="pageSize">The page size when pagination</param>
        /// <param name="reload">If true, load data from database or files, otherwise load from cached file</param>
        /// <returns>A TataTable object</returns>
        public async Task<DataTable> GetTimeSeries(XMLSchemaModel xmlSchemaModel, DataViewModel instanceDataView, string category, string fieldName, TimeSeriesFrequency frequency, TimeSeriesOperation op, int startRow, int pageSize, bool reload)
        {
            DataTable timeSeries = null;
                ...
                if (xmlSchemaModel.RootElement.ElementType == instanceDataView.BaseClass.ClassName)
                {
                    await Task.Factory.StartNew(() =>
                    {
                        ...
                        if (!DataSetHelper.IsEmptyDataSet(ds, instanceDataView.BaseClass.ClassName))
                        {
                            ...
                            TimeSeriesGenerator generator = new TimeSeriesGenerator(_connection.MetaDataModel, _connection.ConnectionString, xmlSchemaModel, instanceView, instanceCount);

                            timeSeries = generator.GetTimeSeries(category, fieldName, frequency, op, startRow, pageSize, reload);
                        }
                    });
                }
                else
        ...
        /// <summary>
        /// Get point count of a time series in xml for the data instance using the provided xml schema
        /// </summary>
        /// <param name="xmlSchemaModel">the xml schema model</param>
        /// <param name="instanceDataView">The dataview model of the instance(s)</param>
        /// <param name="category">The category of a time series, can be null</param>
        /// <returns>An integer</returns>
        public async Task<int> GetTimeSeriesPointCount(XMLSchemaModel xmlSchemaModel, DataViewModel instanceDataView, string category)
        {
            int count = 0;
                ...
                if (xmlSchemaModel.RootElement.ElementType == instanceDataView.BaseClass.ClassName)
                {
                    await Task.Factory.StartNew(() =>
                    {
                        ...
                        if (!DataSetHelper.IsEmptyDataSet(ds, instanceDataView.BaseClass.ClassName))
                        {
                            ...
                            count = generator.GetTimeSeriesPointCount(category);
                        }
                    });
                }
                else
        ...
        /// <summary>
        /// Get categories in a time series metric grouped by a category field in the matrix
        /// </summary>
        /// <param name="xmlSchemaModel">the xml schema model</param>
        /// <param name="instanceDataView">The dataview model of the instance(s)</param>
        /// <returns>A list of categories</returns>
        public async Task<IList<string>> GetTimeSeriesCategories(XMLSchemaModel xmlSchemaModel, DataViewModel instanceDataView)
        {
                ...
                if (xmlSchemaModel.RootElement.ElementType == instanceDataView.BaseClass.ClassName)
                {
                    await Task.Factory.StartNew(() =>
                    {
                        ...
                        if (!DataSetHelper.IsEmptyDataSet(ds, instanceDataView.BaseClass.ClassName))
                        {
                            ...
                            categories = generator.GetTimeSeriesCategories();
                        }
                    });
                }
                else
