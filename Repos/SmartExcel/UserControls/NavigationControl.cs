...
namespace SmartExcel
{
    public partial class NavigationControl : UserControl, IDataGridControl
    {
        ...
        private Newtera.Common.Core.SchemaInfo[] GetSchemaInfos()
        {
            ...
            if (_isAuthenticated)
            {
                ...
                try
                {
                    ...
                    SmartExcel.WorkflowModelWebService.SchemaInfo[] schemas = workflowModelServic.GetAuthorizedSchemaInfos(ConnectionStringBuilder.Instance.Create());
        ...
        private void BeginQueryDone(Object source, SmartExcel.CMDataWebService.BeginQueryCompletedEventArgs e)
        {
        ...
        {
            Microsoft.Office.Tools.Excel.Workbook workBook = Globals.ThisWorkbook;
            workBook.RemoveCustomization();
        }
            ...
            if (!string.IsNullOrEmpty(_xmlSchemaViewName))
            {
                ...
                if (instanceView != null)
                {
                    try
                    {
                        ...
                        // invoke the web service synchronously
                        XmlNode xmlNode = _dataService.GenerateXmlDoc(ConnectionStringBuilder.Instance.Create(_metaData.SchemaInfo),
