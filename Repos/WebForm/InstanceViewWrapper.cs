...
namespace Newtera.WebForm
{
    ...
    public class InstanceViewWrapper : IInstanceWrapper
    {
        ...
        private bool _forceUpdate; // when true, force to update the instance regardless if the values of properties have changed or not, so the update events will be generated
        ...
        /// The instance wrapped by this <see cref="Newtera.Common.Wrapper.InstanceViewWrapper"/>.
        /// </param>
        public InstanceViewWrapper(InstanceView instanceView, string connectionString, dynamic instance)
        {
            _instanceView = instanceView;
            _connection = null;
        ...
        /// The instance wrapped by this <see cref="Newtera.Common.Wrapper.InstanceViewWrapper"/>.
        /// </param>
        public InstanceViewWrapper(InstanceView instanceView, string connectionString, dynamic instance, bool forceUpdate)
        {
            _instanceView = instanceView;
            _connection = null;
            _connectionString = connectionString;
        ...
        /// The instance wrapped by this <see cref="Newtera.Common.Wrapper.InstanceViewWrapper"/>.
        /// </param>
        public InstanceViewWrapper(InstanceView instanceView, CMConnection connection, dynamic instance)
        {
            _instanceView = instanceView;
        ...
        public string GetString(string theProperty)
        {
            ...
            if (IsHitoryEditProperty(attributeName))
            {
                // HistoryEditProperty get value from the UI input field ("text");
                if (_instance != null)
                {
                    string text = "";
                    if (_instance[attributeName] != null &&
                        _instance[attributeName]["text"] != null)
                    {
                        text = _instance[attributeName]["text"];
                    }
                    //var jsonString = JsonConvert.SerializeObject(_instance, Newtonsoft.Json.Formatting.Indented);
            ...
            else if (!IsAttributeNull(attributeName))
            {
        ...
        public void SetValue(string theProperty, object propertyValue)
        {
            try
            {
                ...
                IInstanceWrapper relatedInstance = propertyValue as IInstanceWrapper;
                ...
                if (relatedInstance == null)
                {
                    ...
                    if (_forceUpdate)
                    {
                        // clear the value in the cell of  the dataset so that the property will be updated even
                        // if the value has not been changed, to ensure th eupdate event is fired
                        DataSetHelper.SetCellValue(this.WrappedInstance.DataSet,
                            this.WrappedInstance.DataView.BaseClass.ClassName, attributeName, 0, null);
                    }
                }
                else
                {
                    string relatedTableName = DataRelationshipAttribute.GetRelationshipDataTableName(this.WrappedInstance.DataView.BaseClass.Name, attributeName);

                    // Replace the row in DataTable for the new related instance
                    if (this.WrappedInstance.DataSet != null)
                    {
                        // existing instance
                        if (this.WrappedInstance.DataSet.Tables[relatedTableName] != null)
                        {
                            this.WrappedInstance.DataSet.Tables[relatedTableName].Clear(); // clear the existing rows
                            DataTable dt = relatedInstance.WrappedInstance.DataSet.Tables[relatedInstance.WrappedInstance.DataView.BaseClass.Name];
                            // import the new rows
                            for (int i = 0; i < dt.Rows.Count; i++)
                            {
                                this.WrappedInstance.DataSet.Tables[relatedTableName].ImportRow(dt.Rows[i]);
                            }
                        }
                        else
                        {
                            throw new Exception("Unable to find DataTable for " + relatedTableName);
                        }
                    }
                    else
                    {
                        // new instance
                        this.WrappedInstance.DataSet = new DataSet();
                        DataTable newTable = this.WrappedInstance.DataSet.Tables.Add(relatedTableName);
                        DataTable dt = relatedInstance.WrappedInstance.DataSet.Tables[relatedInstance.WrappedInstance.DataView.BaseClass.Name];
                        // import the new rows
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
        ...
        private string GetAttributeName(string theProperty)
        {
            ...
            if (attributeName == null)
            {
                ...
                {
                    Exception ex = GetException("Unable to find the property with a name or display name as " + theProperty + " in class " + _instanceView.DataView.BaseClass.Caption + ". Please check workflow rule definitions.");
                    throw ex;
                }
                else
                {
        ...
        /// <summary>
        /// Gets the information indicating whether the property is of HistoryEditProperty type
        /// </param>
        /// <returns>true if it is a history edit property, false otherwise.</returns>
        private bool IsHitoryEditProperty(string attributeName)
        {
            bool status = false;
            InstanceAttributePropertyDescriptor ipd = this.PropertyDescriptors[attributeName] as InstanceAttributePropertyDescriptor;
            if (ipd != null && ipd.IsHistoryEdit)
            {
                status = true;
            }
