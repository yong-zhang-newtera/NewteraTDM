/*
* @(#) RelationshipGridViewControl.cs
*
...
namespace Newtera.WebForm
{
    ...
    /// </summary>
    public class RelationshipGridViewControl : PropertyControlBase
    {
        ...
        public override JToken GetPropertyViewModel()
        {
            ...
            // add an empty row at the end of the view model, to allow adding new instance
            if (!IsReadOnly)
            {
                editor = new InstanceEditor();
                editor.EditInstance = new InstanceView(_relatedInstanceView.DataView); // create an instance view with no data
                editor.ConnectionString = this.InstanceEditor.ConnectionString;
                editor.Document = this.InstanceEditor.Document;
                editor.ParentEditor = this.InstanceEditor.ParentEditor;
                ...
                //ErrorLog.Instance.WriteLine("last obj=" + jsonString);
        ...
        public override HtmlNode CreatePropertyNode()
        {
            ...
            if (!this.IsReadOnly)
            {
                string text = WebControlsResourceManager.GetString("PropertyEditor.Command");
                thNode = this.InstanceEditor.Document.CreateElement("td");
                thNode.InnerHtml = text;
                trNode.AppendChild(thNode);
            ...
            for (int i = 0; i < properties.Count; i++)
            {
                ...
                if (!vPropertyInfo.IsRelationship &&
                     !vPropertyInfo.IsImage &&
                     !vPropertyInfo.IsArray)
                {
                    bool readOnly = IsPropertyReadOnly(vPropertyInfo);

                    vPropertyControl = editor.CreatePropertyControl(vPropertyInfo.Name, readOnly, null);
            ...
            string arrayPath = this.InstanceEditor.ArrayBasePath + this.PropertyInfo.Name;

            // delete and add row commands
            if (!this.IsReadOnly)
            {
                // Add btn
                tdNode = this.InstanceEditor.Document.CreateElement("td");
        ...
        // DataViewName is stored in NestedDataViewName attribute of the ClassElement or its parent classes
        private string GetDataViewName()
        {
            string dataViewName = null;
            string baseClassName = this.BaseInstanceView.DataView.BaseClass.ClassName;
            ClassElement classElement = this.BaseInstanceView.DataView.SchemaModel.FindClass(baseClassName);

            _defaultRows = GetDefaultRowCount(classElement);

            while (classElement != null)
            {
                if (!string.IsNullOrEmpty(classElement.NestedDataViewName))
                {
                    dataViewName = classElement.NestedDataViewName;
                    break;
                }
        ...
        private bool IsPropertyReadOnly(InstanceAttributePropertyDescriptor pd)
        {
            bool status = this.IsReadOnly;
