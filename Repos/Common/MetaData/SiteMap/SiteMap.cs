...
namespace Newtera.Common.MetaData.SiteMap
{
	...
	public class SiteMap : ISiteMapNode
	{
		...
		public SiteMap() : base()
		{
            ...
            _rootNode.ParentNode = this;
		}

		/// <summary>
		/// Initiating an instance of SiteMap class
		/// </summary>
		/// <param name="xmlElement">The xml element conatins data of the instance</param>
		internal SiteMap(XmlElement xmlElement) : base()
		{
			this.Unmarshal(xmlElement);
		}

        /// <summary>
        /// Gets the site map node of specified path that consists of node's name,
        /// such as nodeName1/nodeName2
        /// </summary>
        /// <param name="nodePath">The path of node's nodes</param>
        /// <returns>The ISiteMapNode object, null if no exist</returns>
        public ISiteMapNode FindSiteMapNode(string nodePath)
        {
            ISiteMapNode found = null;

            foreach (SiteMapNode node in this.ChildNodes)
            {
                if (node.ToXPath() == nodePath)
                {
                    found = node;
                    break;
                }
                ...
                {
                    foreach (ISiteMapNode child in node.ChildNodes)
                    {
                        if (child.ToXPath() == nodePath)
                        {
                            found = child;
                            break;
                        }
        ...
        /// <summary>
        /// Gets the site map node with a specific hash code.
        /// </summary>
        /// <param name="hashCode">An unique hash code of a node</param>
        /// <returns>The ISiteMapNode object, null if no exist</returns>
        /// <remarks>if the nodeHashCode is zero, return the first child </remarks>
        public ISiteMapNode FindSiteMapNode(int nodeHashCode)
        {
            ISiteMapNode found = null;

            if (nodeHashCode == 0)
            {
                // get the first child node as a default node
                if (this.ChildNodes.Count > 0)
                {
                    found = this.ChildNodes[0];
                }
            }
            else
            {
                foreach (ISiteMapNode node in this.ChildNodes)
                {
                    if (node.GetNodeHashCode() == nodeHashCode)
                    {
                        found = node;
                        break;
                    }
                    else if (node.ChildNodes.Count > 0)
                    {
                        foreach (ISiteMapNode child in node.ChildNodes)
                        {
                            if (child.GetNodeHashCode() == nodeHashCode)
                            {
                                found = child;
                                break;
                            }
        ...
        public string BuildMenuItemPath(string nodePath)
        {
            string menuItemPath = "/Model/siteMapNode[@Name='']";

            string[] nodeNames = nodePath.Split('/');

            foreach (string nodeName in nodeNames)
            {
                menuItemPath += "/siteMapNode[@Name='" + nodeName + "']";
            }
		...
		public void Write(Stream stream)
		{
			...
			{
				throw new SiteMapException("Failed to write the SchemaModel object", ex);
			}
		}
        ...
        {
            if (this._rootNode.ChildNodes != null)
            {
                this._rootNode.ChildNodes.Add(child);
                child.ParentNode = _rootNode;
            }
        }
        ...
        public void DeleteChildNode(ISiteMapNode child)
        {
            if (this._rootNode.ChildNodes != null)
            {
                this._rootNode.ChildNodes.Remove(child);
        ...
        {
            _rootNode = (SiteMapNode)NodeFactory.Instance.Create((XmlElement)parent.ChildNodes[0]);
            _rootNode.ParentNode = this;
        }
        ...
        {
            // write the _rootNode
            XmlElement child = parent.OwnerDocument.CreateElement(NodeFactory.Instance.ConvertTypeToString(_rootNode.NodeType));
            _rootNode.Marshal(child);
		...
		private XmlDocument GetXmlDocument()
		{
            ...
            element.SetAttribute("xmlns", "http://schemas.microsoft.com/AspNet/SiteMap-File-1.0");
            element.SetAttribute("enableLocalization", "true");
        ...
        public virtual string ToDisplayPath()
        {
            if (ParentNode != null)
            {
                return ParentNode.ToDisplayPath() + "/siteMap";
            }
            else
            {
                return "/Model";
        ...
        public virtual string ToXPath()
        {
            if (ParentNode != null)
            {
                return ParentNode.ToXPath() + "/siteMap";
            }
            else
            {
                return "/Model";
