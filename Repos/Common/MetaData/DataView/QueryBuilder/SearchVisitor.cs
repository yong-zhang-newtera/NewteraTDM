...
namespace Newtera.Common.MetaData.DataView.QueryBuilder
{
	...
	internal class SearchVisitor : IDataViewElementVisitor
	{
		private DataViewModel _dataView;
		private ICompositQueryElement _statement;

		/// <summary>
		/// Initiate an instance of SearchVisitor class
		/// </summary>
		/// <param name="dataView"></param>
		public SearchVisitor(DataViewModel dataView)
		{
			_dataView = dataView;
			_statement = new XQueryStatement();
		}
		...
		public ICompositQueryElement XQueryStatement
		{
			get
			{
				return _statement;
		...
		{
			if (element.Type == DataClassType.BaseClass)
			{
				int from = _dataView.PageIndex * _dataView.PageSize + 1;
				int to = from + _dataView.PageSize - 1;
				ICompositQueryElement forClause = new ForInDocumentClause(element.Alias, element.ClassName,
					_dataView.SchemaInfo.Name, _dataView.SchemaInfo.Version,
					from, to);
				_statement.Children.Add((IQueryElement) forClause);
			}
			else if (element.Type == DataClassType.ReferencedClass && element.IsReferenced)
			{
                ICompositQueryElement clause;

                // get the relationship element that references this class
                DataClass referringClass = _dataView.FindClass(element.ReferringClassAlias);
                ClassElement referringClassElement = _dataView.SchemaModel.FindClass(referringClass.ClassName);
                RelationshipAttributeElement referringRelationship = referringClassElement.FindInheritedRelationshipAttribute(element.ReferringRelationshipName);
                if (referringRelationship == null)
                {
                    throw new Exception("Unable to find the relationship " + element.ReferringRelationshipName + " in the class " + referringClass.Name);
                }

                if (!referringRelationship.IsForeignKeyRequired)
                {
                    // the relationship between referring class and this class is one-to-many,
                    // use ForClause to iterate through the instances of the many side
                    clause = new ForClause(element.Alias, element.ClassName,
                        element.ReferringClassAlias, element.ReferringRelationshipName);
                }
                else
                {
                    // the relationship between referring class and this class is many-to-one,
                    // use LetClause to assign the instance to a variable on the one-side
                    clause = new LetClause(element.Alias, element.ClassName,
                            element.ReferringClassAlias, element.ReferringRelationshipName);
                }

                _statement.Children.Add((IQueryElement)clause);
			}
		...
		public bool VisitFilter(Filter element)
		{
			...
			{
				ICompositQueryElement whereClause = new WhereClause();
				whereClause.Children.Add((IQueryElement) element.Expression);
				_statement.Children.Add((IQueryElement) whereClause);
			}
