...
namespace Newtera.Common.MetaData.DataView.Validate
{
    ...
    /// <summary>
    /// The implementation of the <see cref="Newtera.Common.Wrapper.IInstanceWrapper"/>
    /// for an instance kept in a DataViewModel's result attributes.
    /// </summary>
    public class InstanceWrapper : IInstanceWrapper
    {
        #region Fields

        /// <summary>The wrapped DataViewModel.</summary>
        ...
        /// </summary>
        /// <param name="dataView">
        /// The data view model wrapped by this <see cref="Newtera.Common.Wrapper.InstanceWrapper"/>.
        /// </param>
        public InstanceWrapper(DataViewModel dataView)
        {
            _dataView = dataView;
            
            _instanceView = new InstanceView(dataView, null, false, false);
        }
        ...
        {
            if (this.WrappedInstance != null && srcInstance.WrappedInstance != null)
            {
                try
                {
        ...
        {
            PropertyDescriptor ipd = this.PropertyDescriptors[attributeName];
            object val = ipd.GetValue(null);
        ...
        private bool DoesAttributeContains(string attributeName, object attributeValue)
        {
            ...
            if (ipd != null && (ipd.IsMultipleChoice || ipd.IsArray))
            {
                if (ipd.IsMultipleChoice)
                {
                    string stringVal = ipd.GetStringValue();
