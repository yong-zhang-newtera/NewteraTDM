...
namespace Newtera.Server.Engine.Workflow
{
    ...
    public class InstanceWrapper : IInstanceWrapper
    {
        ...
        /// </param>
        public InstanceWrapper(InstanceView instanceView)
        {
        ...
        public DateTime GetDateTime(string theProperty)
        {
            ...
            if (!IsAttributeNull(attributeName))
            {
                ...
                {
                    return DateTime.MinValue; // deafault value
                }
            }
            else
            {
        ...
        public decimal GetDecimal(string theProperty)
        {
            ...
            if (!IsAttributeNull(attributeName))
            {
                ...
                {
                    return decimal.Zero; // deafult value
                }
            }
            else
            {
        ...
        public object GetValue(string theProperty)
        {
            ...
            if (ipd == null)
            {
                throw new Exception("Unable to find the property :" + theProperty);
            }
            if (_instanceView.InstanceData != null)
            {
                ...
                if (val != null)
                {
                    if (ipd.IsRelationship)
                    {
                        if (!string.IsNullOrEmpty(val.ToString()))
                        {
                            // val is PK value, get the referenced instance in wraper
                            NewteraDataService dataService = new NewteraDataService();
                            InstanceView referencedInstanceView = dataService.GetInstanceView(_instanceView.DataView.SchemaInfo.NameAndVersion, ipd.ReferencedClassName, val.ToString());  // val is a primary key of the referenced instance
                            if (referencedInstanceView == null)
                            {
                                throw new Exception("Unable to find the referenced instance for property :" + theProperty + " with the primary key :" + val);
                            }
                            val = new InstanceWrapper(referencedInstanceView);
                        }
                        else
                            val = null;
                    }
                    else if (ipd.PropertyType.IsEnum)
                    {
                    ...
                    else if (ipd.IsArray)
                    {
                        ...
                        DataTable dt = val as DataTable;
                        string[] stringArray = new string[dt.Columns.Count * dt.Rows.Count];
                        int index;
                        for (int row = 0; row < dt.Rows.Count; row++)
                        {
                            for (int col = 0; col < dt.Columns.Count; col++)
                            {
                                index = row * dt.Columns.Count + col;
                                if (!dt.Rows[row].IsNull(col))
                                {
                                    stringArray[index] = dt.Rows[row][col].ToString();
                                }
                                else
                                {
                                    stringArray[index] = "";
        ...
        public void Save()
        {
            if (_instanceView.IsDataChanged)
            {
                try
                {
                    // execute update query
                    string updateQuery = _instanceView.DataView.UpdateQuery;
                    NewteraDataService dataService = new NewteraDataService();
                    // Save is called by PolicyActivity, execute the update query without raising events.
                    dataService.ExecuteNonQuery(null, null, updateQuery, false);
                }
                catch (Exception ex)
                {
                    Exception ex1 = GetException("Save method failed for the binding instance because of " + ex.Message);
                    throw ex1;
                }
            }
        }
        ...
        private Exception GetException(string reason)
        {
            ...
            try
            {
                ...
                NewteraWorkflowInstance workflowInstance = NewteraWorkflowRuntime.Instance.FindWorkflowInstance(wfInstanceId);
                if (workflowInstance != null)
                {
                    workflowInstance.Exception = ex;
                }
            }
            catch (Exception)
            {
                // The instance is not associated with a workflow instance
            }
