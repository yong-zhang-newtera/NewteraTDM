...
namespace Newtera.Server.DB
{
	...
	internal class SQLServerClobDAO : IClobDAO, IDisposable
	{
		private SQLServerProvider _dataProvider;
		private SqlConnection _connection;

		/// <summary>
		/// Initiate a new instance of SQLServerClobDAO.
		/// </summary>
		/// <param name="dataProvider">data provider</param>
		public SQLServerClobDAO(IDataProvider dataProvider)
		{
			_dataProvider = (SQLServerProvider) dataProvider;
			_connection = null;
		}
		...
		public Stream ReadClob(string tableName, string columnName, string rowId)
		{
			...
			try
			{
				...
				if (_connection == null)
				{
					_connection = (SqlConnection) _dataProvider.Connection;
				}
        ...
        public void WriteClob(string text, string tableName, string columnName, string rowId)
        {
            ...
            {
                cmd.CommandText = updateClobSql;
                ((SqlCommand)cmd).Parameters.Add("@id", SqlDbType.BigInt, 20).Value = rowId;
		...
		public void WriteClob(IDbCommand cmd, string dataString, string tableName, string columnName, string objId)
		{
			...
			cmd.CommandText = updateClobSql;
            ((SqlCommand)cmd).Parameters.Clear();
			((SqlCommand) cmd).Parameters.Add("@oid", SqlDbType.BigInt,20).Value = objId;
			((SqlCommand) cmd).Parameters.Add("@text", SqlDbType.NText).Value = dataString;
			cmd.ExecuteNonQuery();
		}
        ...
        public void WriteBlob(string tableName, string columnName, string idName, string rowId, byte[] data)
        {
            ...
            try
            {
                ...
                cmd.Parameters.Add("@id", SqlDbType.NVarChar, 200).Value = rowId;
                ...
                cmd.CommandText = updateClobSql;
                ptrParm = cmd.Parameters.Add("@pointer", SqlDbType.Binary, 16);
                ptrParm.Value = pointer;
                SqlParameter attachmentParm = cmd.Parameters.Add("@bytes", SqlDbType.Image, bufferLen);
		...
		public void WriteAttachmentBlob(AttachmentType attachmentType, Stream stream, string attachmentId)
		{
			...
			{
				cmd.CommandText = getBlobPtrSql;
				cmd.Parameters.Add("@id", SqlDbType.NVarChar, 100).Value = attachmentId;
				SqlParameter ptrParm = cmd.Parameters.Add("@pointer", SqlDbType.Binary, 16);
				ptrParm.Direction = ParameterDirection.Output;
