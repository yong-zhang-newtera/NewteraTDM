...
namespace Newtera.Server.DB
{
	...
	public class MetaDataAdapter
	{
		...
		/// <summary>
		/// Gets the information indicating whether the database has been enabled for
		/// full-text search
		/// </summary>
		...
		public bool IsFullTextEnabled()
		{
			...
			IDbCommand cmd = con.CreateCommand();
			IDataReader reader = null;
			string sql = CannedSQLManager.GetCannedSQLManager(_dataProvider).GetSql("GetFullTextEnableStatus");
            ...
            if (!string.IsNullOrEmpty(sql))
            {
                try
                {
                    ...
                    if (reader.Read())
                    {
                        if (!reader.IsDBNull(0))
                        {
                            string val = reader.GetValue(0).ToString().Trim();
                            switch (_dataProvider.DatabaseType)
                            {
                                ...
                                case DatabaseType.Oracle:
                                    ...
                                    // find the entry in the database which indicates
                                    // the full-text has been enabled.
