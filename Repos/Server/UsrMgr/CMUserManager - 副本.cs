...
namespace Newtera.Server.UsrMgr
{
	...
	public class CMUserManager : IUserManager
	{
        ...
        private const string AddUserQuery1 = "let $u := [[<User xmlns:xsi=\"http://www.w3.org/2003/XMLSchema-instance\" xsi:type=\"User\"><ID>param1</ID><Password>param2</Password><LastName>param3</LastName><FirstName>param4</FirstName><Email>param5</Email></User>]] return addInstance(document(\"db://UserInfo.xml\"), $u)";
        ...
        private const string ChangeUserDataQuery = "for $u in document(\"db://UserInfo.xml\")/UserList/User where $u/ID = \"param1\" return (setText($u/LastName, \"param2\"), setText($u/FirstName, \"param3\"), setText($u/Email, \"param4\"), updateInstance(document(\"db://UserInfo.xml\"), $u))";
        ...
        public UserRecord GetUserInfoByUserKey(string userKey)
        {
            ...
            string[] userData = GetUserData(userKey);
        ...
        /// <summary>
        /// Gets users that belongs to a role,
        /// </summary>
        ...
        public string[] GetUsers(string roleName)
        {
            ...
            // first try to get users from the cache
            users = UserDataCache.Instance.GetRoleUsers(roleName);
            if (users == null)
            {
                IPrincipal originalPrincipal = Thread.CurrentPrincipal;
                QueryReader reader = null;

                // This method can be called while executing a query to get role's users.
                // ignore the call if the query is in process.
                if (!_isQueryInProcess)
                {
                    try
                    {
                        ...
                        string query = GetRoleUsersQuery;
                        ...
                        while (doc.DocumentElement.ChildNodes.Count > 0)
                        {
                            foreach (XmlElement element in doc.DocumentElement.ChildNodes)
                            {
                                userCollection.Add(element.InnerText);
                            }
                ...
                // remember in cache for performance sake
                UserDataCache.Instance.SetRoleUsers(roleName, users);
            }
