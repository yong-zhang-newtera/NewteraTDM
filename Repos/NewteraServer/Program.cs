...
using Newtera.WebApi;
using Newtera.Common.Core;

namespace NewteraServer
{
    public class Program
    {
        static int Main(string[] args)
        {
            var exitCode = HostFactory.Run(x =>
            {
                ...
                x.RunAsLocalSystem();
                x.SetDescription("Newtera Server as Windows service");
                x.SetDisplayName("Newtera Server");
                x.SetServiceName("Newtera.Newtera.Server");
            });
            return (int)exitCode;
        }
    }
